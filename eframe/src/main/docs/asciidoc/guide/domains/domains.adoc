...
[inline-toc]#<<Locking>>#
[inline-toc]#<<SQL Database Definition>>#
...
See <<domain-testing,Domain Testing>> for details on testing.
...
NOTE: Do not call the repository `save()`, `update()` or `delete()` methods directly.  There
...
* A UUID identifier with the name _'uuid'_.  DB column type is _uuid_.
...
<<efDefinitionList>> marker in your pages like this:

[source,html]
----
<@efDefinitionList columns="order,product,status,qtyToBuild"/>
----
...
coding simpler.  For example, the <<efDefinitionList>> marker relies on the key field
...
<<reference.adoc#efshow,efShow>> markers in your HTML files.  This means updating 3  files
whenever you add a new field to a domain class.  This is your choice.

See link:reference.html#field-order[fieldOrder^] icon:share-square-o[role="link-blue"]
for more details.

==== clientDefaults

To make life easier for the end users, we provide several mechanisms for default values.
The simplest is the default value in the domain fields (e.g. _String title="ABC"_).
...
To support this client-side default mechanism, you can add a static property to your domain
to provide the default values.  For example, the fields for <<Flex Types>> uses as sequence
default to reduce the user input on the client.  This is defined in the FlexField using
this definition:

[source,groovy]
.Example - clientDefaults
----
class FlexField {
  BasicFieldFormat fieldFormat = StringFieldFormat.instance  // <.>
  HistoryTracking historyTracking = HistoryTracking.NONE

  static clientDefaults = [sequence:  "_max('sequence')+10",  // <.>
                           fieldName: "'field'+(_max('sequence')+10)"] // <.>
}

----
<.> Normal server-side default value for two enum fields.
<.> Uses the current max 'sequence' from the inline grid rows and adds 10.
<.> Creates a string `fieldName` using like _'field20'_.
...
NOTE: The clientDefaults will override the normal field default values when
...
This means you can't use this annotation in most test classes or in POGO/POJO logic.
...
----
interface LSNSequenceRepository extends CrudRepository<LSNSequence, UUID> {
...
[inline-toc]#<<DB Creation Scripts>>#
[inline-toc]#<<Indexes and Constraints>>#
...
The database for production/development/test is Postgres SQL.  This framework uses the
https://flywaydb.org/[Flyway Database Migration^] for creation and migration of the SQL schema
used in Postgres.  The DDL scripts are stored in these directories.
...
Flyway will only run a given SQL file against the schema one time.  If you later change the file,
...
files are unique.  This allows developers to add new tables during development of the 1.2 release.

NOTE: You can use the Flyway `repair` command to help with this issue.

===== Indexes and Constraints

SQL databases use indexes and constraints to help control relationships between tables.
For the framework and application modules, we try to use these base rules:

* *PRIMARY KEY* - This is the UUID for the record.  Every record has one (except for many-to-many
                  join tables).
* *FOREIGN KEY* - A UUID field that references a record in another table.  This key almost
                  always has an index created for it.  This is usually implemented as a *REFERENCES*
...
Non-nullable references can use the LEFT join logic:
...
interface ChildRepository extends CrudRepository<Child, UUID> {
...
By default, this short string is usually just the first key field (as a string).
The link:groovydoc/org/simplemes/eframe/misc/TypeUtils.html[TypeUtils^]
icon:share-square-o[role="link-blue"] utility class provides an easy way to get this short string.
...
NOTE: If you are using a domain as a reference in a grid, then you should also implement
...
You can also use _Map_ as the result class in the query.  This makes simple SQL _COUNT(*)_ cases
easier to use:

[source,groovy]
.Example - PreparedStatement - Simple Count Scenario
----

def tableName = DomainEntityHelper.instance.getTableName(Order)  // <.>
def list = SQLUtils.instance.executeQuery("SELECT COUNT(*) as count FROM ${tableName}", Map) // <.>

def count = list[0].count  // <.>

----
<.> The _Order_ table name is _ordr_.
<.> We want to count the total number of records in the table.
<.> The count is saved in a map in row 0, with a name of _'count'_.  The column name
